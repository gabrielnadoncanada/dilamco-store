/* ============================================================
   Dilamco — Module Renderer (SVG cabinet illustrations)
   Generates a stylized SVG of any module given its dimensions,
   color, molding style, family. Used in cards & PDP.
   ============================================================ */
...
function ModuleRender({ product, color, molding, scale = 1 }) {
  ...
  const isWall = family.includes('murale') || family.includes('au-dessus');
  const moldingW = molding === '3 po' ? 0.16 : 0.07; // relative

  // Map module proportions to a viewBox.
  // Horizontal scale less aggressive so very narrow units don't disappear.
  const aspectW = Math.max(0.5, w / Math.max(w, h)) * 100;
  const aspectH = Math.max(0.5, h / Math.max(w, h)) * 100;

  const viewW = 200;
  const viewH = 240;

  // Cabinet box dimensions in viewBox units
  const padX = 30;
  const padY = 20;
  const boxW = viewW - padX * 2;
  const boxH = viewH - padY * 2;
  // Scale box to module proportions
  const ratio = w / h;
  let dw, dh;
  if (ratio >= 1) {
    dw = boxW;
  ...
  // Pantry: split horizontally into top + bottom door usually
  let panels = [];
  if (isPantry && h >= 60) {
    // Two stacked doors
    const splitY = y0 + dh * 0.5;
    panels.push({ x: x0 + doorPadding, y: y0 + doorPadding, w: dw - doorPadding * 2, h: (dh / 2) - doorPadding * 1.5 });
    panels.push({ x: x0 + doorPadding, y: splitY + doorPadding * 0.5, w: dw - doorPadding * 2, h: (dh / 2) - doorPadding * 1.5 });
  } else {
  ...
  return (
    <svg viewBox={`0 0 ${viewW} ${viewH}`} width="100%" height="100%" preserveAspectRatio="xMidYMid meet" style={{ overflow: 'visible' }}>
      ...
      {panels.map((p, i) => {
        ...
        const knobX = isWideDoor ? p.x + p.w * 0.5 : (i % 2 === 0 ? p.x + p.w - 5 : p.x + 5);
      ...
      <text x={viewW/2} y={y0 + dh + 22} textAnchor="middle" fontSize="7" fontFamily="ui-monospace, monospace" fill="rgba(60,55,40,0.6)" letterSpacing="0.5">
        {w}″
      </text>
      {isCorner && (
        <g>
...
function DoorPanel({ x, y, w, h, color, moldingW }) {
  // Shaker: outer frame + inset panel
  ...
  return (
    <g>
      ...
      {/* Subtle highlight on top frame */}
      <rect x={x} y={y} width={w} height="0.6" fill="rgba(255,255,255,0.12)" />
    </g>
  );
}

window.ModuleRender = ModuleRender;
